...
const ImageBanner = ({ title, button = null, description, description2 = null, imageUrl, imagePos = "left", onClick }: ImageBannerProps) => {
    const [ref, isVisible] = useOnScreen();
    const [isHovered, setIsHovered] = React.useState(false);
    return (
        <div
            ref={ref}
            className={`relative overflow-hidden transition-opacity duration-700 min-h-[400px] bg-[#fdf8f7] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 bg-[#fdf8f7]"
                }`}
        >
            {isVisible && (
                <div className="relative w-full h-[50vh] overflow-hidden">
                    ...
                    <div className={`relative z-10 flex justify-${imagePos === 'left' ? 'start' : 'end'} items-center h-full w-full bg-black/30`}>
                        <motion.div
