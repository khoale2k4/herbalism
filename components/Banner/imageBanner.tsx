...
const ImageBanner = ({
    ...
    return (
        <div
            ref={ref}
            className={`relative transition-opacity duration-700 min-h-[400px] bg-[#fdf8f7] ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
                }`}
        >
            {isVisible && (
                <div className="relative w-full h-auto overflow-hidden">
                    {/* Background image with motion on all screens */}
                    <motion.div
                        initial={{ scale: 1.2 }}
                        animate={{ scale: 1 }}
                    ...
                    {/* Desktop overlay content with motion */}
                    <div className={`absolute inset-0 hidden md:flex z-10 justify-${imagePos === 'left' ? 'start' : 'end'} items-center bg-black/30`}>
                        <motion.div
                        ...
                        >
                            ...
                            {button && (
                                ...
                                >
                                    ...
                                    <div className={`absolute top-0 left-0 w-full h-full bg-[#3e4f3d] transform transition-transform duration-300 ease-out ${isHovered ? 'translate-x-0' : '-translate-x-full'}`}></div>
                                </button>
                            )}
                        </motion.div>
                    </div>

                    {/* Mobile content (non-motion, under image) */}
                    <div className="block md:hidden px-4 py-6 bg-white text-center">
                        <h2 className="text-lg font-semibold mb-2">{title}</h2>
                        <p className="text-gray-700 mb-2">{description}</p>
                        {description2 && <p className="text-gray-700 mb-2">{description2}</p>}
                        {button && (
                            <button
                                onClick={onClick}
                                className="mt-4 px-4 py-2 bg-[#3e4f3d] text-white text-sm font-medium rounded-md"
                            >
                                {button}
                            </button>
                        )}
